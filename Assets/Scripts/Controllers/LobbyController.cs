...
public class LobbyController : MonoBehaviour
{
    ...
    {
        // CORRECTIF race condition : s'abonner aux events de scène dès que NOTRE propre
        // connexion NGO est confirmée (pas uniquement lors de StartClient/JoinRelayAutoAsync
        // qui confirme seulement le transport, pas le handshake Netcode complet).
        // Sans ce fix, le client AR peut manquer le SceneEvent.Load envoyé par le Host
        // et rester bloqué en LobbyScene même quand la partie démarre.
        if (clientId == NetworkManager.Singleton.LocalClientId)
        {
            EnsureSceneEventListener();
            Debug.Log($"[LobbyController] Connexion NGO confirmée (clientId={clientId}). Abonné aux events de scène.");
        }

        // Si nous sommes l'Hôte (VR) et que le client (AR) se connecte
        if (NetworkManager.Singleton.IsServer && clientId != NetworkManager.Singleton.LocalClientId)
        {
            Debug.Log($"[LobbyController] Joueur AR connecté (clientId={clientId}) ! Lancement de la scène de jeu.");
